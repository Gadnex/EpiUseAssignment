...
public class FiveCardDrawHand implements Hand {
    ...
    @Override
    public void addCard(Card card) throws HandException {
        if (cards.size() == 5) {
            throw new HandException("A maximum of 5 cards may be assigned to a hand");
        }
        cards.add(card);
    }

    @Override
    public HandStrenth getHandStrength() {
        if (isStraightFlush()) {
            return HandStrenth.STRAIGHT_FLUSH;
        }
        if (isFourOfKind()) {
            return HandStrenth.FOUR_OF_A_KIND;
        }
        if (isFullHouse()) {
            return HandStrenth.FULL_HOUSE;
        }
        if (isFlush()) {
            return HandStrenth.FLUSH;
        }
        if (isStraight()) {
            return HandStrenth.STRAIGHT;
        }
        if (isThreeOfKind()) {
            return HandStrenth.THREE_OF_A_KIND;
        }
        if (isTwoPair()) {
            return HandStrenth.TWO_PAIR;
        }
        if (isOnePair()) {
            return HandStrenth.ONE_PAIR;
        }
        return HandStrenth.HIGH_CARD;
    }
